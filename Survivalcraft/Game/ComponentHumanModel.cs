...
namespace Game
{
	public class ComponentHumanModel : ComponentCreatureModel
	{
		...
		private DrawBlockEnvironmentData m_drawBlockEnvironmentData = new DrawBlockEnvironmentData();
		...
		private ModelBone m_hand1Bone;

		private ModelBone m_hand2Bone;

		private float m_sneakFactor;

		private float m_lieDownFactorEye;

		private float m_lieDownFactorModel;

		private float m_walkAnimationSpeed;

		private float m_walkLegsAngle;

		private float m_walkBobHeight;

		private float m_headingOffset;

		private float m_punchFactor;

		private float m_punchPhase;

		private int m_punchCounter;

		private float m_footstepsPhase;

		private bool m_rowLeft;

		private bool m_rowRight;

		private float m_aimHandAngle;

		private Vector3 m_inHandItemOffset;

		private Vector3 m_inHandItemRotation;

		private Vector2 m_headAngles;

		private Vector2 m_handAngles1;

		private Vector2 m_handAngles2;

		private Vector2 m_legAngles1;

		private Vector2 m_legAngles2;
