...
namespace Game
{
	...
	{
		private class Geometry : TerrainGeometry
		{
			public Geometry()
			{
		...
		private CellFace m_cellFace;

		private int m_value;

		private object m_highlightRaycastResult;

		private static int[] m_drawOrders = new int[2]
		{
			1,
			2000
		};
		...
		private void DrawRayHighlight(Camera camera)
		{
		...
		private void DrawReticleHighlight(Camera camera)
		{
		...
		private void DrawFillHighlight(Camera camera)
		{
			if (camera.Eye.HasValue && m_highlightRaycastResult is TerrainRaycastResult)
			{
		...
		private void DrawOutlineHighlight(Camera camera)
		{
		...
		private static void DrawBoundingBoxFace(FlatBatch3D batch, int face, Vector3 c1, Vector3 c2, Color color)
		{
			switch (face)
			{
			case 0:
		...
		private BoundingBox GetCellFaceBoundingBox(Point3 point)
		{
			int cellValue = m_subsystemTerrain.Terrain.GetCellValue(point.X, point.Y, point.Z);
