...
namespace Game
{
	...
	{
		private class MovingBlockSet : IMovingBlockSet
		{
			public string Id;
			...
			public Point3 GeometryGenerationPosition = new Point3(2147483647);
			...
			void IMovingBlockSet.SetBlock(Point3 offset, int value)
			{
				...
				UpdateBox();
				GeometryGenerationPosition = new Point3(2147483647);
			}
		...
		private SubsystemAnimatedTextures m_subsystemAnimatedTextures;

		private List<MovingBlockSet> m_movingBlockSets = new List<MovingBlockSet>();

		private List<MovingBlockSet> m_stopped = new List<MovingBlockSet>();

		private List<MovingBlockSet> m_removing = new List<MovingBlockSet>();

		private DynamicArray<TerrainVertex> m_vertices = new DynamicArray<TerrainVertex>();

		private DynamicArray<ushort> m_indices = new DynamicArray<ushort>();

		private DynamicArray<IMovingBlockSet> m_result = new DynamicArray<IMovingBlockSet>();

		private Shader m_shader;

		private BlockGeometryGenerator m_blockGeometryGenerator;

		private bool m_canGenerateGeometry;
