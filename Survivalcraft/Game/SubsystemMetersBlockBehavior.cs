...
namespace Game
{
	public class SubsystemMetersBlockBehavior : SubsystemBlockBehavior, IUpdateable
	{
		...
		private Dictionary<Point3, int> m_thermometersByPoint = new Dictionary<Point3, int>();

		private DynamicArray<Point3> m_thermometersToSimulate = new DynamicArray<Point3>();

		private int m_thermometersToSimulateIndex;

		private const int m_diameterBits = 6;

		private const int m_diameter = 64;

		private const int m_diameterMask = 63;

		private const int m_radius = 32;

		private DynamicArray<int> m_toVisit = new DynamicArray<int>();

		private int[] m_visited = new int[8192];
