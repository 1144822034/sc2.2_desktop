...
namespace Game
{
	public class ComponentPathfinding : Component, IUpdateable
	{
		...
		private SubsystemPathfinding m_subsystemPathfinding;
		...
		private Vector3? m_lastPathfindingDestination;

		private double? m_lastPathfindingTime;

		private float m_pathfindingCongestion;

		private PathfindingResult m_pathfindingResult = new PathfindingResult();

		private double m_nextUpdateTime;

		private int m_randomMoveCount;

		private bool m_destinationChanged;

		private const float m_minPathfindingPeriod = 8f;

		private const float m_pathfindingCongestionCapacity = 500f;

		private const float m_pathfindingCongestionCapacityLimit = 1000f;

		private const float m_pathfindingCongestionDecayRate = 20f;
