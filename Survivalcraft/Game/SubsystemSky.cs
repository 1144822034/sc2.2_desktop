...
namespace Game
{
	public class SubsystemSky : Subsystem, IDrawable, IUpdateable
	{
		...
		private class SkyDome : IDisposable
		{
		...
		private Color m_viewFogColor;

		private Vector2 m_viewFogRange;

		private bool m_viewIsSkyVisible;

		private Texture2D m_sunTexture;

		private Texture2D m_glowTexture;

		private Texture2D m_cloudsTexture;

		private Texture2D[] m_moonTextures = new Texture2D[8];

		private static UnlitShader m_shaderFlat = new UnlitShader(useVertexColor: true, useTexture: false, useAlphaThreshold: false);

		private static UnlitShader m_shaderTextured = new UnlitShader(useVertexColor: true, useTexture: true, useAlphaThreshold: false);

		private VertexDeclaration m_skyVertexDeclaration = new VertexDeclaration(new VertexElement(0, VertexElementFormat.Vector3, VertexElementSemantic.Position), new VertexElement(12, VertexElementFormat.NormalizedByte4, VertexElementSemantic.Color));

		private Dictionary<GameWidget, SkyDome> m_skyDomes = new Dictionary<GameWidget, SkyDome>();

		private VertexBuffer m_starsVertexBuffer;

		private IndexBuffer m_starsIndexBuffer;

		private VertexDeclaration m_starsVertexDeclaration = new VertexDeclaration(new VertexElement(0, VertexElementFormat.Vector3, VertexElementSemantic.Position), new VertexElement(12, VertexElementFormat.Vector2, VertexElementSemantic.TextureCoordinate), new VertexElement(20, VertexElementFormat.NormalizedByte4, VertexElementSemantic.Color));

		private const int m_starsCount = 150;

		private Vector3? m_lightningStrikePosition;

		private float m_lightningStrikeBrightness;

		private double m_lastLightningStrikeTime;
		...
		private int[] m_drawOrders = new int[3]
		{
			-100,
			5,
			105
		...
		private float[] m_cloudsLayerRadii = new float[4]
		{
			0f,
			0.8f,
			0.95f,
		...
		private Color[] m_cloudsLayerColors = new Color[5];

		private static int[] m_lightValuesMoonless = new int[6]
		{
			0,
			3,
			6,
		...
		private static int[] m_lightValuesNormal = new int[6]
		{
			3,
			5,
			8,
		...
		private void DrawSunAndMoon(Camera camera)
		{
			float globalPrecipitationIntensity = m_subsystemWeather.GlobalPrecipitationIntensity;
			float timeOfDay = m_subsystemTimeOfDay.TimeOfDay;
		...
		private void DrawLightning(Camera camera)
		{
		...
		private void DrawClouds(Camera camera)
		{
		...
		private void UpdateLightAndViewParameters()
		{
		...
		private Color CalculateSkyColor(Vector3 direction, float timeOfDay, float precipitationIntensity, int temperature)
		{
		...
		private void FillSkyVertexBuffer(SkyDome skyDome, float timeOfDay, float precipitationIntensity, int temperature)
		{
			for (int i = 0; i < 8; i++)
			{
		...
		private void FillSkyIndexBuffer(SkyDome skyDome)
		{
			int num = 0;
			for (int i = 0; i < 7; i++)
			{
