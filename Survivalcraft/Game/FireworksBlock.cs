...
namespace Game
{
	public class FireworksBlock : Block
	{
		...
		private BlockMesh[] m_headBlockMeshes = new BlockMesh[64];

		private BlockMesh[] m_bodyBlockMeshes = new BlockMesh[2];

		private BlockMesh[] m_finsBlockMeshes = new BlockMesh[2];

		private static readonly string[] ShapeDisplayNames = new string[8]
		{
			"Small Burst",
			"Large Burst",
			"Circle",
		...
		private static readonly string[] HeadNames = new string[8]
		{
			"HeadConeSmall",
			"HeadConeLarge",
			"HeadCylinderSmall",
		...
		public override string GetDisplayName(SubsystemTerrain subsystemTerrain, int value)
		{
			...
			int altitude = GetAltitude(data);
			bool flickering = GetFlickering(data);
			return string.Format("Fireworks {0} {1}{2} ({3})", FireworksColorDisplayNames[color], flickering ? "Flickering " : null, ShapeDisplayNames[(int)shape], (altitude == 0) ? "Low" : "High");
		}
