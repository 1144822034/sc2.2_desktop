...
namespace Game
{
	public class SubsystemSaplingBlockBehavior : SubsystemBlockBehavior, IUpdateable
	{
		...
		private SaplingData LoadSaplingData(string data)
		{
		...
		private string SaveSaplingData(SaplingData saplingData)
		{
		...
		private void MatureSapling(SaplingData saplingData)
		{
		...
		private bool GrowTree(int x, int y, int z, TreeType treeType)
		{
		...
		private void AddSapling(SaplingData saplingData)
		{
			m_saplings[saplingData.Point] = saplingData;
			m_enumerator = m_saplings.Values.GetEnumerator();
		}

		private void RemoveSapling(Point3 point)
		{
