...
namespace Game
{
	public class SubsystemFurnitureBlockBehavior : SubsystemBlockBehavior
	{
		...
		private List<FurnitureSet> m_furnitureSets = new List<FurnitureSet>();

		private FurnitureDesign[] m_furnitureDesigns = new FurnitureDesign[1024];

		private Dictionary<Point3, List<FireParticleSystem>> m_particleSystemsByCell = new Dictionary<Point3, List<FireParticleSystem>>();
			...
			else
			{
				...
				while (stack.Count > 0)
				{
					...
					if (IsValueAllowed(cellValue))
					{
						...
						stack.Push(new Point3(key.X - 1, key.Y, key.Z));
						stack.Push(new Point3(key.X + 1, key.Y, key.Z));
						stack.Push(new Point3(key.X, key.Y - 1, key.Z));
						stack.Push(new Point3(key.X, key.Y + 1, key.Z));
						stack.Push(new Point3(key.X, key.Y, key.Z - 1));
						stack.Push(new Point3(key.X, key.Y, key.Z + 1));
					}
				}
