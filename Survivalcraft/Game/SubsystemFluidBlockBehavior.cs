...
namespace Game
{
	...
	{
		private static Point2[] m_sideNeighbors = new Point2[4]
		{
			new Point2(-1, 0),
			new Point2(1, 0),
			new Point2(0, -1),
		...
		private FluidBlock m_fluidBlock;

		private Dictionary<Point3, bool> m_toUpdate = new Dictionary<Point3, bool>();

		private Dictionary<Point3, int> m_toSet = new Dictionary<Point3, int>();

		private Dictionary<Point3, int> m_visited = new Dictionary<Point3, int>();

		private Dictionary<Point3, Vector2> m_fluidRandomFlowDirections = new Dictionary<Point3, Vector2>();

		private bool m_generateSources;

		protected SubsystemTime SubsystemTime
		{
			get;
			private set;
		}

		protected SubsystemAudio SubsystemAudio
		{
			get;
			private set;
		}

		protected SubsystemAmbientSounds SubsystemAmbientSounds
		{
			get;
			private set;
		}
		...
		private void UpdateIsTop(int value, int x, int y, int z)
		{
		...
		protected virtual bool OnFluidInteract(int interactValue, int x, int y, int z, int fluidValue)
		{
		...
		protected float? CalculateDistanceToFluid(Vector3 p, int radius, bool flowingFluidOnly)
		{
		...
		protected void Set(int x, int y, int z, int value)
		{
		...
		private void FlowTo(int x, int y, int z, int level)
		{
		...
		private int LevelAtNearestFall(int x, int y, int z, int level, Dictionary<Point3, int> levels)
		{
			if (level > m_fluidBlock.MaxLevel)
			{
				return 2147483647;
