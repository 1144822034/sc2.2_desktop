...
namespace Game
{
	public class InventorySlotWidget : CanvasWidget, IDragTargetWidget
	{
		...
		private RectangleWidget m_highlightWidget;

		private BlockIconWidget m_blockIconWidget;

		private LabelWidget m_countWidget;

		private ValueBarWidget m_healthBarWidget;

		private RectangleWidget m_editOverlayWidget;

		private RectangleWidget m_interactiveOverlayWidget;

		private RectangleWidget m_foodOverlayWidget;

		private LabelWidget m_splitLabelWidget;

		private GameWidget m_gameWidget;

		private DragHostWidget m_dragHostWidget;
		...
		private InventoryDragData m_inventoryDragData;
		...
		private bool HandleMoveItem(IInventory sourceInventory, int sourceSlotIndex, IInventory targetInventory, int targetSlotIndex, int count)
		{
		...
		private bool HandleDragDrop(IInventory sourceInventory, int sourceSlotIndex, DragMode dragMode, IInventory targetInventory, int targetSlotIndex)
		{
			int slotValue = sourceInventory.GetSlotValue(sourceSlotIndex);
			int slotValue2 = targetInventory.GetSlotValue(targetSlotIndex);
