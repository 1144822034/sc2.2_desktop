...
namespace Game
{
	public class SubsystemPathfinding : Subsystem
	{
		...
		private class World : IAStarWorld<Vector3>
		{
			...
			private float GetBlockWalkingHeight(Block block, int value)
			{
			...
			private bool IsBlocked(BoundingBox box)
			{
		...
		private AStar<Vector3> m_astar = new AStar<Vector3>();
		...
		private void SmoothPath(DynamicArray<Vector3> path, Vector3 boxSize)
		{
		...
		private bool IsPassable(Vector3 p1, Vector3 p2, Vector3 boxSize)
		{
		...
		private static bool SmoothingRaycastFunction_Obstacle(int value, float distance)
		{
		...
		private static bool SmoothingRaycastFunction_Support(int value, float distance)
		{
			int num = Terrain.ExtractContents(value);
			Block block = BlocksManager.Blocks[num];
			if (block.ShouldAvoid(value))
