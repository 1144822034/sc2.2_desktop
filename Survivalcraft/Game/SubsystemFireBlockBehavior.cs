...
namespace Game
{
	public class SubsystemFireBlockBehavior : SubsystemBlockBehavior, IUpdateable
	{
		...
		private Dictionary<Point3, float> m_expansionProbabilities = new Dictionary<Point3, float>();

		private Dictionary<Point3, FireData> m_fireData = new Dictionary<Point3, FireData>();

		private DynamicArray<Point3> m_firePointsCopy = new DynamicArray<Point3>();

		private Dictionary<Point3, float> m_toBurnAway = new Dictionary<Point3, float>();

		private Dictionary<Point3, float> m_toExpand = new Dictionary<Point3, float>();

		private int m_copyIndex;

		private float m_remainderToScan;

		private double m_lastScanTime;

		private float m_lastScanDuration;

		private Random m_random = new Random();

		private float m_fireSoundVolume;

		private float m_fireSoundIntensity;
