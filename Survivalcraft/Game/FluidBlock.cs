...
namespace Game
{
	public abstract class FluidBlock : CubeBlock
	{
		...
		private bool[] m_theSameFluidsByIndex;
		...
		protected void GenerateFluidTerrainVertices(BlockGeometryGenerator generator, int value, int x, int y, int z, Color sideColor, Color topColor, TerrainGeometrySubset[] subset)
		{
		...
		private static float ZeroSubst(float v, float subst)
		{
			if (v != 0f)
			{
				return v;
		...
		private static float CalculateFluidVertexHeight(float h1, float h2, float h3, float h4)
		{
