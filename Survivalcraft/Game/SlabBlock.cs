...
namespace Game
{
	public abstract class SlabBlock : Block, IPaintableBlock
	{
		...
		private int m_fullBlockIndex;

		private BlockMesh m_standaloneColoredBlockMesh = new BlockMesh();

		private BlockMesh m_standaloneUncoloredBlockMesh = new BlockMesh();

		private BlockMesh[] m_coloredBlockMeshes = new BlockMesh[2];

		private BlockMesh[] m_uncoloredBlockMeshes = new BlockMesh[2];

		private BoundingBox[][] m_collisionBoxes = new BoundingBox[2][];

		protected SlabBlock(int coloredTextureSlot, int fullBlockIndex)
		{
