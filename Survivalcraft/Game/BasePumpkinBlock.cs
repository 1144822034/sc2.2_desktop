...
namespace Game
{
	...
	{
		protected BlockMesh[] m_blockMeshesBySize = new BlockMesh[8];

		protected BlockMesh[] m_standaloneBlockMeshesBySize = new BlockMesh[8];

		private BoundingBox[][] m_collisionBoxesBySize = new BoundingBox[8][];

		private bool m_isRotten;
		...
		public override string GetDisplayName(SubsystemTerrain subsystemTerrain, int value)
		{
			...
			if (m_isRotten)
			{
				if (size >= 7)
				{
					return "Rotten Pumpkin";
				}
				return "Rotten Unripe Pumpkin";
			}
			if (size >= 7)
			{
				return "Pumpkin";
