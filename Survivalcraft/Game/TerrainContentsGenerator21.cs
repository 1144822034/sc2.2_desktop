...
namespace Game
{
	public class TerrainContentsGenerator21 : ITerrainContentsGenerator
	{
		...
		{
			private static int m_counter = (int)Stopwatch.GetTimestamp();
		...
		private class OldRandom
		{
			...
			private InternalRandom m_random;

			private static int m_seed = (int)Stopwatch.GetTimestamp();

			public static readonly OldRandom GlobalRandom = new OldRandom(0);

			public OldRandom()
			{
				m_random = new InternalRandom(997 * m_seed++);
			}

			public OldRandom(int seed)
			{
				m_random = new InternalRandom(seed);
			}
