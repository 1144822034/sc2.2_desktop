...
namespace Game
{
	public class PaintBucketBlock : BucketBlock
	{
		...
		private BlockMesh m_standaloneBucketBlockMesh = new BlockMesh();

		private BlockMesh m_standalonePaintBlockMesh = new BlockMesh();
			...
			while (color < 16)
			{
				...
				for (int additive = 0; additive < 4; additive = num2)
				{
					...
					if (num != color)
					{
						...
						{
							Description = $"Make {SubsystemPalette.GetName(null, num, null)} Paint",
		...
		public override string GetDisplayName(SubsystemTerrain subsystemTerrain, int value)
		{
			...
			return SubsystemPalette.GetName(subsystemTerrain, color, "Paint Bucket");
		}
