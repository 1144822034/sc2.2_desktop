...
namespace Game
{
	public class ComponentBody : ComponentFrame, IUpdateable
	{
		...
		private DynamicArray<CollisionBox> m_collisionBoxes = new DynamicArray<CollisionBox>();

		private DynamicArray<ComponentBody> m_componentBodies = new DynamicArray<ComponentBody>();

		private DynamicArray<IMovingBlockSet> m_movingBlockSets = new DynamicArray<IMovingBlockSet>();

		private DynamicArray<CollisionBox> m_bodiesCollisionBoxes = new DynamicArray<CollisionBox>();

		private DynamicArray<CollisionBox> m_movingBlocksCollisionBoxes = new DynamicArray<CollisionBox>();

		private ComponentBody m_parentBody;

		private List<ComponentBody> m_childBodies = new List<ComponentBody>();

		private Vector3 m_velocity;

		private bool m_isSneaking;

		private Vector3 m_totalImpulse;

		private Vector3 m_directMove;

		private bool m_fluidEffectsPlayed;

		private float m_stoppedTime;

		private static Vector3[] m_freeSpaceOffsets;
		...
		private void UpdateImmersionData()
		{
			Vector3 position = base.Position;
		...
		private void MoveWithCollision(float dt, Vector3 move)
		{
			Vector3 position = base.Position;
		...
		private bool HandleSmoothRise(ref Vector3 move, Vector3 position, float dt)
		{
			Vector3 boxSize = BoxSize;
		...
		private void HandleAxisCollision(int axis, float move, ref Vector3 position, bool isSmoothRising)
		{
			Vector3 boxSize = BoxSize;
			m_collisionBoxes.Clear();
		...
		private void FindBodiesCollisionBoxes(Vector3 position, DynamicArray<CollisionBox> result)
		{
		...
		private void FindMovingBlocksCollisionBoxes(Vector3 position, DynamicArray<CollisionBox> result)
		{
			Vector3 boxSize = BoxSize;
		...
		private void FindTerrainCollisionBoxes(BoundingBox box, DynamicArray<CollisionBox> result)
		{
		...
		private void FindSneakCollisionBoxes(Vector3 position, Vector2 overhang, DynamicArray<CollisionBox> result)
		{
		...
		private bool IsColliding(BoundingBox box, DynamicArray<CollisionBox> collisionBoxes)
		{
		...
		private float CalculatePushBack(BoundingBox box, int axis, DynamicArray<CollisionBox> collisionBoxes, out CollisionBox pushingCollisionBox)
		{
		...
		private float CalculateSmoothRisePushBack(BoundingBox normalBox, BoundingBox smoothRiseBox, int axis, DynamicArray<CollisionBox> collisionBoxes, out CollisionBox pushingCollisionBox)
		{
			pushingCollisionBox = default(CollisionBox);
			float num = 0f;
			for (int i = 0; i < collisionBoxes.Count; i++)
		...
		private static float CalculateBoxBoxOverlap(ref BoundingBox b1, ref BoundingBox b2, int axis)
		{
		...
		private static float GetVectorComponent(Vector3 v, int axis)
		{
			switch (axis)
			{
			case 0:
		...
		private static void InelasticCollision(float v1, float v2, float m1, float m2, float cr, out float result1, out float result2)
		{
			float num = 1f / (m1 + m2);
