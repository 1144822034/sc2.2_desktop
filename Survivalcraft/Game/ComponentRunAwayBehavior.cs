...
namespace Game
{
	public class ComponentRunAwayBehavior : ComponentBehavior, IUpdateable, INoiseListener
	{
		...
		private ComponentFrame m_attacker;

		private float m_timeToForgetAttacker;

		private bool m_heardNoise;

		private Vector3? m_lastNoiseSourcePosition;
		...
		private float ScoreSafePlace(Vector3 currentPosition, Vector3 safePosition, Vector3? herdPosition, Vector3? noiseSourcePosition, int contents)
		{
			float num = 0f;
