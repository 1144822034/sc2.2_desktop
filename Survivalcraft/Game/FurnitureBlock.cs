...
namespace Game
{
	public class FurnitureBlock : Block, IPaintableBlock, IElectricElementBlock
	{
		...
		private Matrix[] m_matrices = new Matrix[4];

		private int[][] m_facesMaps = new int[4][]
		{
			new int[6]
			{
				0,
		...
		private int[][] m_reverseFacesMaps = new int[4][]
		{
			new int[6]
			{
				0,
		...
		{
			DialogsManager.ShowDialog(null, new MessageDialog("Error", "Too many different furniture designs", "OK", null, null));
		}
