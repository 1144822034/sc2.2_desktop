...
namespace Game
{
	public class SoundGeneratorElectricElement : RotateableElectricElement
	{
		...
		private SoundParticleSystem m_particleSystem;

		private Random m_random = new Random();

		private int m_lastToneInput;

		private double m_playAllowedTime;

		private string[] m_tones = new string[16]
		{
			"",
			"Bell",
			"Organ",
		...
		private int[] m_maxOctaves = new int[16]
		{
			0,
			5,
			5,
		...
		private string[] m_drums = new string[10]
		{
			"Snare",
			"BassDrum",
			"ClosedHiHat",
