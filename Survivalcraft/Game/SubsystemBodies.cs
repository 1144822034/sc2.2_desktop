...
namespace Game
{
	...
	{
		private const float m_areaSize = 8f;

		private DynamicArray<ComponentBody> m_componentBodies = new DynamicArray<ComponentBody>();

		private Dictionary<ComponentBody, Point2> m_areaByComponentBody = new Dictionary<ComponentBody, Point2>();

		private Dictionary<Point2, DynamicArray<ComponentBody>> m_componentBodiesByArea = new Dictionary<Point2, DynamicArray<ComponentBody>>();
		...
		public BodyRaycastResult? Raycast(Vector3 start, Vector3 end, float inflateAmount, Func<ComponentBody, float, bool> action)
		{
			...
			bodyRaycastResult.Distance = 3.40282347E+38f;
		...
		private void ComponentBody_PositionChanged(ComponentFrame componentFrame)
		{
