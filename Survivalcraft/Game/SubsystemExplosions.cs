...
namespace Game
{
	...
	{
		private class SparseSpatialArray<T>
		{
			private const int m_bits1 = 4;

			private const int m_bits2 = 4;

			private const int m_mask1 = 15;

			private const int m_mask2 = 15;

			private const int m_diameter = 256;
		...
		private SparseSpatialArray<SurroundingPressurePoint> m_surroundingPressureByPoint;

		private int m_projectilesCount;

		private Dictionary<Projectile, bool> m_generatedProjectiles = new Dictionary<Projectile, bool>();

		private Random m_random = new Random();

		private ExplosionParticleSystem m_explosionParticleSystem;
		...
		private void SimulateExplosion(int x, int y, int z, float pressure, bool isIncendiary)
		{
		...
		private void TryAddPoint(int x, int y, int z, int axis, float currentPressure, bool isIncendiary, List<ProcessPoint> toProcess, SparseSpatialArray<bool> processed)
		{
		...
		private void PostprocessExplosions(bool playExplosionSound)
		{
		...
		private void CalculateImpulseAndDamage(ComponentBody componentBody, float? obstaclePressure, out Vector3 impulse, out float damage)
		{
		...
		private void CalculateImpulseAndDamage(Vector3 position, float mass, float? obstaclePressure, out Vector3 impulse, out float damage)
		{
