...
namespace Game
{
	...
	{
		private static List<TerrainBrush>[] m_treeBrushesByType;

		private static int[] m_treeTrunksByType;

		private static int[] m_treeLeavesByType;
		...
		private static TerrainBrush CreateTreeBrush(Random random, int woodIndex, int leavesIndex, int height, int branchesCount, Func<int, float> leavesProbabilityByHeight, Func<int, float> branchesLengthByHeight)
		{
