...
namespace Game
{
	public class PressurePlateBlock : MountedElectricElementBlock
	{
		...
		private BlockMesh[] m_standaloneBlockMeshesByMaterial = new BlockMesh[2];

		private BlockMesh[] m_blockMeshesByData = new BlockMesh[16];

		private BoundingBox[][] m_collisionBoxesByData = new BoundingBox[16][];

		private string[] m_displayNamesByMaterial = new string[2]
		{
			"Wooden Pressure Plate",
			"Stone Pressure Plate"
		};

		private int[] m_creativeValuesByMaterial = new int[2]
		{
			Terrain.MakeBlockValue(144, 0, 0),
			Terrain.MakeBlockValue(144, 0, 1)
		};

		private int[] m_textureSlotsByMaterial = new int[2]
		{
			4,
			1
		};
		...
		public override void GenerateTerrainVertices(BlockGeometryGenerator generator, TerrainGeometry geometry, int value, int x, int y, int z)
		{
			...
			if (num < m_blockMeshesByData.Length && m_blockMeshesByData[num] != null)
			{
				...
				generator.GenerateWireVertices(value, x, y, z, GetFace(value), 0.8125f, Vector2.Zero, geometry.SubsetOpaque);
			}
		}
