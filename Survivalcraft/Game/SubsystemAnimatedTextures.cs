...
namespace Game
{
	public class SubsystemAnimatedTextures : Subsystem, IUpdateable
	{
		...
		private SubsystemBlocksTexture m_subsystemBlocksTexture;

		private RenderTarget2D m_animatedBlocksTexture;

		private PrimitivesRenderer2D m_primitivesRenderer = new PrimitivesRenderer2D();

		private ScreenSpaceFireRenderer m_screenSpaceFireRenderer = new ScreenSpaceFireRenderer(200);

		private Random m_random = new Random();

		private bool m_waterOrder;

		private Vector2 m_waterOffset1;

		private Vector2 m_waterOffset2;

		private bool m_magmaOrder;

		private Vector2 m_magmaOffset1;

		private Vector2 m_magmaOffset2;

		private double m_lastAnimateGameTime;
		...
		private void AnimateWaterBlocksTexture()
		{
		...
		private void AnimateMagmaBlocksTexture()
		{
			TexturedBatch2D batch = m_primitivesRenderer.TexturedBatch(m_subsystemBlocksTexture.BlocksTexture, useAlphaTest: false, 0, DepthStencilState.None, null, BlendState.AlphaBlend, SamplerState.PointClamp);
		...
		private Rectangle AnimateFireBlocksTexture(float dt)
		{
			...
			m_screenSpaceFireRenderer.MinTimeToLive = 1f / 0f;
			m_screenSpaceFireRenderer.MaxTimeToLive = 1f / 0f;
		...
		private void DrawBlocksTextureSlot(TexturedBatch2D batch, int slotX, int slotY, Vector2 tc1, Vector2 tc2, Vector2 tcOffset, Color color)
		{
