namespace Game
{
	...
	{
		private int m_lastChangeCircuitStep;

		private bool m_needsReset;

		private float m_voltage;

		public TrapDoorElectricElement(SubsystemElectricity subsystemElectricity, CellFace cellFace)
			...
			if (!m_needsReset)
			{
				if (num >= 10)
				{
					if (ElectricElement.IsSignalHigh(m_voltage))
					{
						...
						base.SubsystemElectricity.Project.FindSubsystem<SubsystemTrapdoorBlockBehavior>(throwOnError: true).OpenCloseTrapdoor(cellFace.X, cellFace.Y, cellFace.Z, !TrapdoorBlock.GetOpen(data));
					}
				}
				else
				{
