...
namespace Game
{
	public class ComponentFirstPersonModel : Component, IDrawable, IUpdateable
	{
		...
		private Model m_handModel;

		private Vector3? m_lastYpr;

		private Vector2 m_lagAngles;

		private float m_swapAnimationTime;

		private float m_pokeAnimationTime;

		private Vector3 m_itemOffset;

		private Vector3 m_itemRotation;

		private double m_nextHandLightTime;

		private float m_handLight;

		private int m_itemLight;
		...
		private static LitShader m_shader = new LitShader(2, useEmissionColor: false, useVertexColor: false, useTexture: true, useFog: false, useAlphaThreshold: false);
