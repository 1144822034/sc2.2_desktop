...
namespace Game
{
	...
	{
		private class ModelData : IComparable<ModelData>
		{
		...
		private SubsystemShadows m_subsystemShadows;

		private PrimitivesRenderer3D m_primitivesRenderer = new PrimitivesRenderer3D();

		private static ModelShader m_shaderOpaque = new ModelShader(useAlphaThreshold: false, 7);

		private static ModelShader m_shaderAlphaTested = new ModelShader(useAlphaThreshold: true, 7);

		private Dictionary<ComponentModel, ModelData> m_componentModels = new Dictionary<ComponentModel, ModelData>();

		private List<ModelData> m_modelsToPrepare = new List<ModelData>();

		private List<ModelData>[] m_modelsToDraw = new List<ModelData>[4]
		{
			new List<ModelData>(),
			new List<ModelData>(),
			new List<ModelData>(),
		...
		private int[] m_drawOrders = new int[4]
		{
			-10000,
			1,
			99,
		...
		private void PrepareModel(ModelData modelData, Camera camera)
		{
		...
		private void DrawModels(Camera camera, List<ModelData> modelsData, float? alphaThreshold)
		{
			DrawInstancedModels(camera, modelsData, alphaThreshold);
			DrawModelsExtras(camera, modelsData);
		}

		private void DrawInstancedModels(Camera camera, List<ModelData> modelsData, float? alphaThreshold)
		{
		...
		private void DrawModelsExtras(Camera camera, List<ModelData> modelsData)
		{
			foreach (ModelData modelsDatum in modelsData)
			{
		...
		private float? CalculateModelLight(ModelData modelData)
		{
			Vector3 p;
