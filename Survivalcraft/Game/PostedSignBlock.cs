...
namespace Game
{
	public abstract class PostedSignBlock : SignBlock, IElectricElementBlock, IPaintableBlock
	{
		...
		private int m_attachedSignBlockIndex;

		private BlockMesh m_standaloneBlockMesh = new BlockMesh();

		private BlockMesh m_standaloneColoredBlockMesh = new BlockMesh();

		private BlockMesh[] m_blockMeshes = new BlockMesh[16];

		private BlockMesh[] m_coloredBlockMeshes = new BlockMesh[16];

		private BlockMesh[] m_surfaceMeshes = new BlockMesh[16];

		private Vector3[] m_surfaceNormals = new Vector3[16];

		private BoundingBox[][] m_collisionBoxes = new BoundingBox[16][];

		private Vector3[] m_directions = new Vector3[16];

		protected PostedSignBlock(string modelName, int coloredTextureSlot, int attachedSignBlockIndex)
		{
