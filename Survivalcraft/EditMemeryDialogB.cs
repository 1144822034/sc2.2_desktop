...
namespace Game
{
    public class EditMemeryDialogB : Dialog
    {
        ...
        public Widget initButton()
        {
            ...
            for (int i = 0; i < 6; i++)
            {
                ...
                for (int j = 0; j < 3; j++)
                {
                    int pp = i * 3 + j;
                    if (pp >= 16) continue;
                    ClickTextWidget clickText = new ClickTextWidget(new Vector2(50, 40), string.Format("{0}", MemoryBankData.m_hexChars[pp]), delegate () {
                        AudioManager.PlaySound("Audio/UI/ButtonClick", 1f, 0f, 0f);
                    ...
                    stackPanelWidget.Children.Add(clickText);
        ...
        public Widget makeButton(string txt, Action tas)
        {
            ...
            clickTextWidget.rectangleWidget.FillColor = Color.Gray;
            clickTextWidget.rectangleWidget.OutlineThickness = 2;
            clickTextWidget.Margin = new Vector2(0, 2);
