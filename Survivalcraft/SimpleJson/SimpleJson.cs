		...
		private static string ParseString(char[] json, ref int index, ref bool success)
		{
			...
			while (!flag && index != json.Length)
			{
				...
				case '\\':
				{
					...
					{
						index += 4;
						if (json.Length - index < 6 || !(new string(json, index, 2) == "\\u") || !uint.TryParse(new string(json, index + 2, 4), NumberStyles.HexNumber, CultureInfo.InvariantCulture, out uint result2) || 56320 > result2 || result2 > 57343)
						{
							success = false;
							return "";
						}
